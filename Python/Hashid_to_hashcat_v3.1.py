...
#   - hashcat --identify    (fallback when hashid returns no modes)
#
# USAGE:
...
#   - Apr26 (v3.1): Falls back to `hashcat --identify` when hashid resolves no modes (e.g. BitLocker)
# =============================================================================
...
def hashcat_identify(h):
    """Fallback: use hashcat --identify to resolve mode directly."""
    r = run(["hashcat", "--identify", h])
    if r.returncode != 0:
        return []
    modes = []
    for line in r.stdout.splitlines():
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="hc_runs_") as td:
        for h in hashes:
            ...
            if not modes:
                print(f"# hashid found no modes, trying hashcat --identify for {h}")
                modes = hashcat_identify(h)
                if not modes:
