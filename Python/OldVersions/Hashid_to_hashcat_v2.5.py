...
import os, re, sys, tempfile, subprocess, threading, time, termios, tty, select
...
def run_hashcat_try(hash_str, mode, wordlist, outfile, status_buf_lock, status_buf):
    # capture stdout to buffer; quiet banners; request periodic status
    ...
    def reader():
        ...
        for line in proc.stdout:
            # collect only status blocks triggered by periodic timer or 's'
            block.append(line.rstrip("\n"))
            if line.strip() == "":
                with status_buf_lock:
                    status_buf[:] = block[-40:]  # keep last ~40 lines
        proc.stdout.close()
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="hc_runs_") as td:
        for h in hashes:
            ...
            if err: print(f"# ERROR for {h}: {err}", file=sys.stderr); continue
            if not cands: print(f"# No candidates: {h}"); continue

            modes = []
            for name, explicit in cands: modes.extend(resolve_modes(name, explicit, hc_idx))
            seen=set(); deduped=[]
            for m in modes:
                if m not in seen: seen.add(m); deduped.append(m)
            modes = deduped
            if not modes: print(f"# No modes resolved: {h}"); continue
            ...
            for m in modes:
                ...
                if run_hashcat_try(h, m, wordlist, outfile, status_buf_lock, status_buf):
                    found = True
                    if STOP_ON_FIRST_FOR_HASH: break
            # silent if not found
